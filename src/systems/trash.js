...
AFRAME.registerSystem('trash', {
  ...
  init() {
    ...
    this.isGameRunning = false; // Thêm trạng thái theo dõi trò chơi
    this.el.addEventListener('setGameState', (event) => {
      this.isGameRunning = event.detail.gameState === 'in-progress';
      if (!this.isGameRunning) {
        this.stopTrashWave(); // Dừng tạo rác khi trò chơi kết thúc
        this.dropTrash(); // Xóa rác còn sót lại
      }
    });
  },

  startTrashWave() {
    if (!this.isGameRunning) return; // Không bắt đầu nếu trò chơi không đang chạy
    this.delay = 4000;
    this.nextTrash();
  },
  ...
  nextTrash() {
    if (!this.isGameRunning) return; // Không tạo rác nếu trò chơi không đang chạy
    this.levelTrash++;
    if (this.timeout) {
      clearTimeout(this.timeout);
      this.timeout = null;
  ...
  createTrash(trashData) {
    if (!this.isGameRunning) return; // Chặn việc tạo rác nếu trò chơi không đang chạy
